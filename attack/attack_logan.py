...
result_dir = f'./logan_result/zdim{opt.nz}/datasetSize{opt.data_num}/'
...
# for e in range(5000,100001,5000):
    epochlist.append(e)
    wb_predictions = []
    wb_prob = []
    wb_label = []
    ...
    torch.save(wb_prob, 'd_score.pkl')
    exit()
...
# exit()
torch.save(aucroclist, os.path.join(result_dir,'1g2d_d2.pkl'))
exit()
...
plt.clf()
plt.ylim(0.4,1)
plt.plot(epochTestList, aucroclist, label='aucroc WGAN')
plt.plot(epochTestList, aucroclist2, label='aucroc mixWGAN')
# plt.plot(epochlist, aucprclist, label='aucprc')
plt.legend()
plt.savefig(result_dir + "aucLogan_iteration.png")
