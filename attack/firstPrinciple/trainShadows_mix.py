...
if __name__ == "__main__":
    ...
    for shadowID in range(257,512):
        ...
        dataloader2 = torch.utils.data.DataLoader(dataset=dataset, batch_size=bs, shuffle=True)
        ...
        result_dir = f'./models/mix/{shadowID}/images/'
        os.makedirs(result_dir,exist_ok=True)
        store_param = f'./models/mix/{shadowID}/params/'
        os.makedirs(store_param,exist_ok=True)

        loss_fn = torch.nn.BCEWithLogitsLoss()
        for epoch in range(opt.n_epochs):
            for i, ((imgs, cond,_),(imgs2, cond2,_)) in enumerate(zip(dataloader,dataloader2)):
                real_imgs = imgs.cuda()
