...
if __name__ == "__main__":
    ...
    for shadowID in range(1):
        print("model id:", shadowID)
        dataset = torch.utils.data.Subset(datasetT, datasetIdx[shadowID])
        datasetTup = torch.utils.data.random_split(dataset, [len(dataset)//4, len(dataset)//4, len(dataset)//4, len(dataset)//4])
        dataloaderTup = tuple(torch.utils.data.DataLoader(d, batch_size=bs) for d in datasetTup)

        generator = GanGenerator(z_dim=latent_dim, y_dim=n_class)
        generator = nn.DataParallel(generator).cuda()
        discList = []
        for i in range(4):
            discList.append(GanDiscriminator(y_dim=n_class))
            discList[i] = nn.DataParallel(discList[i]).cuda()

        optimizer_G = torch.optim.Adam(generator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
        optimizer_D_Tup = tuple(torch.optim.Adam(discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2)) for discriminator in discList)

        result_dir = f'./models/{prtmode}/{shadowID}/images/'
        os.makedirs(result_dir,exist_ok=True)
        store_param = f'./models/{prtmode}/{shadowID}/params/'
        os.makedirs(store_param,exist_ok=True)

        loss_fn = torch.nn.BCEWithLogitsLoss()
        for epoch in range(opt.n_epochs):
            for i, dataTup in enumerate(zip(*dataloaderTup)): # (imgs, cond, imgidx)
                for did in range(4):
                    imgs, cond, imgidx = dataTup[did]
                    discriminator = discList[did]
                    optimizer_D = optimizer_D_Tup[did]
                    real_imgs = imgs.cuda()
                    real_label = torch.ones(real_imgs.shape[0]).cuda()
                    fake_label = torch.zeros(real_imgs.shape[0]).cuda()
                    # cond = cond[:,target_idx].cuda()
                    cond = torch.nn.functional.one_hot(cond[:,target_idx], n_class)
                    ...
                    # Adversarial loss
                    # d_loss = torch.pow(fake_validity - real_validity - ref_thres, 2) + lambda_gp * gp; d_loss = torch.mean(d_loss) #  - real_validity + fake_validity
                    # d_loss = torch.mean(relexMask * real_validity) - torch.mean(relexMask * fake_validity) + lambda_gp * gp
                ...
                if i % 50 == 0:
                    ...
                    print(
                        ...
                        % (epoch, opt.n_epochs, i, len(dataloaderTup[0]), d_loss.item(), g_loss.item(), "deprecated")
                        # loss_div.item()
                    )
            ...
            if epoch % 10 == 0:
                ...
                for i in range(4):
                    torch.save(discList[i].module.state_dict(), store_param + 'D_{}_{}.pkl'.format(i, epoch))
