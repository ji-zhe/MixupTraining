...
def gradient_penalty(x, y, c, disc):
    # interpolation
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_iters", type=int, default=30000, help="number of iterations of training")
    ...
    parser.add_argument("--disc_num", type=int, default=2, help="num of discriminators")
    ...
    for i in range(disc_num):
        ...
        discriminatorList.append(discriminator)


    # train_attr = '/home/lxiang_stu8/LFW/dataset/pub_attri.csv'
    # train_fig = '/home/lxiang_stu8/LFW/dataset/public'
    ...
    datasetList = torch.utils.data.random_split(dataset, [opt.data_num//disc_num]*(disc_num) + [len(dataset)-opt.data_num//disc_num*disc_num])
    datasetList = datasetList[:-1]
    dataloaderList = []
    for i in range(disc_num):
        dataloader = torch.utils.data.DataLoader(datasetList[i], bs)
        dataloaderList.append(dataloader)
    # import pdb;pdb.set_trace()

    result_dir = './models/zdim{}/datasetSize{}/PARGAN_model/discNum{}/images/'.format(opt.latent_dim, opt.data_num,opt.disc_num)
    os.makedirs(result_dir, exist_ok=True)
    store_param = './models/zdim{}/datasetSize{}/PARGAN_model/discNum{}/params/'.format(opt.latent_dim, opt.data_num,opt.disc_num)
    ...
    for discriminator in discriminatorList:
        ...
        optimizerDlist.append(optimizer_D)
    ...
    while batches_done < opt.n_iters:
        for i, data in enumerate(zip(*dataloaderList)):
            epoch += 1
            # imgs, cond = list(zip(data))
            # Configure input
            for Did in range(disc_num):
                imgs, cond = data[Did]
                real_imgs = imgs.cuda()
                ...
                # Real images
                real_validity = discriminatorList[Did](real_imgs,cond)
                fake_validity = discriminatorList[Did](fake_imgs, cond)
                gp = gradient_penalty(real_imgs.data, fake_imgs.data, cond.data, discriminatorList[Did])
                # Adversarial loss
                d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + lambda_gp * gp

                optimizerDlist[Did].zero_grad()
                d_loss.backward()
                optimizerDlist[Did].step()
            ...
            if (batches_done + 1) % opt.n_critic == 0:
                ...
                print(
                    ...
                    % (epoch, opt.n_epochs, i, len(dataloader), d_loss.item(), g_loss.item(), "deprecated") #loss_div.item()
                )
                print("gp is {}".format(gp))

            batches_done += 1

            if batches_done % (opt.n_critic*100) == 0:
                torch.save(generator.module.state_dict(), store_param + 'G_{}.pkl'.format(batches_done))
                for Did in range(disc_num):
                    torch.save(discriminatorList[Did].module.state_dict(), store_param + 'D{}_{}.pkl'.format(Did,batches_done))
