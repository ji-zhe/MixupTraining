...
prtmode = 'relaxLoss_changeAllSign'
...
if __name__ == "__main__":
    ...
    reference_thresholds = jload('./jsonfile/base_outLoss_mu_sigma.json')
    reference_thresholds = torch.tensor(reference_thresholds)[:,0]
    ...
    for shadowID in range(3):
        ...
        for epoch in range(opt.n_epochs):
            for i, (imgs, cond, imgidx) in enumerate(dataloader):
                real_imgs = imgs.cuda()
                ...
                diff_real_fake = -real_validity + fake_validity
                ref_thres = reference_thresholds[imgidx].cuda()
                relexMask = diff_real_fake < ref_thres
                relexMask = relexMask * 2 - 1
                relexMask = relexMask.detach() # 1 for overfitted imgs, -1 for normal
